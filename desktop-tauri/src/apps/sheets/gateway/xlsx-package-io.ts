...
import { dirName, type HostIo, joinPath } from './host-io'
...
export interface StreamingSaveRequest {
  ...
  /** fs access for temp dirs / plan content files (see host-io.ts) */
  readonly io: HostIo
  readonly sourcePath: string
  readonly targetPath: string
...
export async function readArchiveEntryText(
  ...
  const workDir = await io.mkdtemp('ai-excel-read-')
  try {
    ...
    await io.remove(workDir, { recursive: true })
  }
}
...
export async function saveWorkbookViaSidecar(
  ...
  const io = request.io
  const workDir = await io.mkdtemp('ai-excel-save-')
  const temporaryTarget = joinPath(
    dirName(request.targetPath),
    `.${crypto.randomUUID()}.tmp.xlsx`,
  )
  try {
    ...
    const replacements = await writePlanContents(io, workDir, 'replace', plan.replaced)
    const additions = [
      ...(await writePlanContents(io, workDir, 'add', plan.added)),
      ...(await writePlanContents(io, workDir, 'add-bin', plan.addedBinary)),
    ]
    ...
    await io.rename(temporaryTarget, request.targetPath)
    return {
    ...
    await io.remove(temporaryTarget).catch(() => undefined)
    throw error
  } finally {
    await io.remove(workDir, { recursive: true }).catch(() => undefined)
  }
}
...
function createSidecarEntrySource(
  ...
  return {
    ...
    readText: async (path) => {
      ...
      const extractDir = joinPath(workDir, `extract-${extractionCount}`)
      extractionCount += 1
      await io.mkdir(extractDir)
      const extracted = readEntriesResultSchema.parse(
...
async function writePlanContents(
  ...
  for (const [name, content] of contents) {
    const contentPath = joinPath(workDir, `${prefix}-${index}.bin`)
    index += 1
    await io.writeFile(contentPath, content)
