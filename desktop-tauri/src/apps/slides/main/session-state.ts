/**
 * Shared slides-session state, ported from the Electron main process
 * (desktop/apps/slides/src/main/session-state.ts) into the webview: per-editor
 * sessions, snapshot undo/redo history, and RenderSlide rebuild helpers.
 *
 * Port notes:
 *  - Electron window references (windowRefs/dialogParent/runtime paths) are
 *    gone: one webview hosts one slides editor, and dialogs are bridge-level
 *    stubs. The `sessions` map is kept (keyed by a host-assigned id) so the
 *    ported host logic stays close to upstream.
 *  - Font metrics come from the ported ./fonts (system-font scan via the Rust
 *    list_fonts command under Tauri; heuristic fallback elsewhere).
 *  - makeMediaResolver base64-encodes with a local helper instead of Buffer.
 */
...
// One session per hosted editor (upstream keyed this by webContents.id; the
// Tauri shell hosts exactly one slides editor per webview, and the host module
// uses a constant key).
...
// The document's source of truth lives in the host (deck.slides mutated in place +
