 ...
 * both drift from the webview's actual rendered width; HarfBuzz shares its shaping engine lineage
 ...
 *
 * Port notes (from the Electron main process into the webview):
 *  - The wasm bytes load via a `?url` asset + fetch (upstream read them with node:fs);
 *    init failure stays fail-safe (null → heuristic metrics).
 *  - System font bytes come from the bridge byte-store under Tauri and node:fs
 *    under vitest; a plain browser finds no fonts and always falls back.
 *  - refineComplexWidths no longer needs webContents.executeJavaScript — the port
 *    IS the renderer, so the canvas measureText pass runs directly in-realm.
 */
...
import hbWasmUrl from '../../../../node_modules/harfbuzzjs/dist/harfbuzz.wasm?url'
...
/** 'darwin' | 'win32' | other — process.platform under Node/vitest, UA guess in the webview */
...
/** Per-platform script -> (font file, CSS family name, coverage-check sample char). The family name must be resolvable by the webview by name. */
...
/** Read a system font file's bytes: node:fs under vitest, the bridge byte-store under Tauri, null elsewhere. */
async function readFontBytes(path: string): Promise<Uint8Array | null> {
  try {
...
/**
 * Renderer-measured ground-truth widths (@100px, scaled linearly). HarfBuzz estimates still
 * deviate from the webview's actual rendering (AAT/morx fonts, font matching cannot be replicated),
...
export function initShapedMetrics(): void {
  ...
  readyPromise = (async () => {
    try {
      ...
      for (const entry of FONT_TABLE[hostPlatform()] ?? []) {
        if (fonts.has(entry.script)) continue
        try {
          const bytes = await readFontBytes(entry.file)
          if (!bytes) continue
...
/** Width (px) of complex-script text: prefer the measured cache; on miss, record it and return the HarfBuzz estimate; null if not applicable. */
...
/**
 * Measure complex-script strings that missed during layout with the page's own canvas
 * measureText (same engine measures and draws — upstream bounced this through
 * webContents.executeJavaScript; the port runs in the renderer already) and backfill the
 * cache. Returns true when new widths were stored, meaning the caller should rebuild the
 * layout. If the measurement fails, keep them pending and retry next time.
 */
export function refineComplexWidths(): Promise<boolean> {
  if (!gtPending.size) return Promise.resolve(false)
