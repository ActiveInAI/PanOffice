...
declare global {
  interface Window {
    slidesApi: SlidesApi
    // NOTE (Tauri port): no `desktop: DesktopFilesApi` here — the shell's docs
    // bridge already declares Window.desktop (DesktopApi), whose attachment
    // subset covers this app's file-API surface; declaring it again with a
    // different type breaks interface merging.
  }
}
