/**
 * window.desktopApi argument/response validation — ported 1:1 from the sheets
 * Electron preload (desktop/apps/sheets/src/preload/index.ts, everything below
 * the desktopApi object). Keeping the layer intact means a malformed bridge
 * request or a corrupt sidecar payload fails with the exact same errors the
 * Electron build produced.
 *
 * The parse* functions are exported for src/bridge/sheets-api.ts; nothing
 * here touches Electron, Node, or Tauri.
 */
import type {
  AiChatResponse,
  AiSettings,
...
export function parseAttachmentAddResult(input: unknown): AttachmentAddResult {
  if (
    !isRecord(input) ||
...
export function parseWorkbookFile(input: unknown): WorkbookFile {
...
export function parseRangeRequest(input: WorkbookRangeRequest): WorkbookRangeRequest {
  if (
    !isRecord(input) ||
...
export function parseFormulaCellsRequest(input: unknown): WorkbookFormulaCellsRequest {
  if (
    !isRecord(input) ||
    !isUuid(input.sessionId) ||
    typeof input.sheetId !== 'string' ||
...
export function parseFormulaCellsResult(input: unknown): WorkbookFormulaCellsResult {
  if (
    !isRecord(input) ||
...
export function parseRecalcRequest(input: unknown): WorkbookRecalcRequest {
  if (
    !isRecord(input) ||
    !isUuid(input.sessionId) ||
...
export function parseRecalcResult(input: unknown): WorkbookRecalcResult {
...
export function parseRangeResult(input: unknown): WorkbookRangeResult {
  if (
    !isRecord(input) ||
    !Array.isArray(input.cells) ||
...
export function parseSaveRequest(input: WorkbookSaveRequest): WorkbookSaveRequest {
  if (
    !isRecord(input) ||
    !isUuid(input.sessionId) ||
...
export function isPdfPageSize(input: unknown): boolean {
...
export function parseSaveResult(input: unknown): WorkbookSaveResult {
...
export function parseMediaRequest(input: WorkbookMediaRequest): WorkbookMediaRequest {
...
export function parseMediaResult(input: unknown): WorkbookMediaResult {
  if (
    !isRecord(input) ||
...
export function parsePivotRequest(input: WorkbookPivotRequest): WorkbookPivotRequest {
...
export function parsePivotDefinitionResult(input: unknown): WorkbookPivotDefinition {
  if (
    !isRecord(input) ||
...
export function isOptionalString(input: unknown): input is string | undefined {
