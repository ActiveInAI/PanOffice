...
import {
  ...
  type BlockBox,
  type TableRowBox,
} from '../../src/apps/docs/renderer/pagination'
import { computeLineMetrics } from '../../src/apps/docs/renderer/line-metrics'
...
// Port note (Electron→Tauri): the corpus stays upstream (read-only); only the
// generated report lands here, under test-results/.
const UPSTREAM_CORPUS = join(__dirname, '../../../desktop/apps/docs/tests/pagination-corpus')
const CORPUS_DIR = join(UPSTREAM_CORPUS, 'docx')
const BASELINE_FILE = join(UPSTREAM_CORPUS, 'baseline-lo.json')
const BASELINE_WORD_FILE = join(UPSTREAM_CORPUS, 'baseline-word.json')
const REPORT_FILE = join(__dirname, '../../test-results/pagination-parity-report.md')
