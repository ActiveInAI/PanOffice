...
import { applyCfRules, buildDxfXml, iconSetSaveable } from '../../src/apps/sheets/gateway/xlsx-cf'

const SHEET = '<worksheet><sheetData><row r="1"><c r="A1"><v>1</v></c></row></sheetData>'
...
describe('applyCfRules', () => {
  it('replaces every existing section with the snapshot', () => {
    ...
    const xml = applyCfRules(SHEET, [{
      ranges: [range],
      stopIfTrue: false,
      rule: { type: 'highlightCell', subType: 'number', operator: 'greaterThan', value: 5, style: { bg: { rgb: '#FFF2CC' } } },
    }], dxfs)
    ...
    expect(dxfs.entries[0]).toBe('<dxf><fill><patternFill><bgColor rgb="FFFFF2CC"/></patternFill></fill></dxf>')
  })
  ...
  it('serializes text, dedup, rank, average, and expression rules', () => {
    ...
    const xml = applyCfRules(SHEET, [
      { ranges: [range], stopIfTrue: true, rule: { type: 'highlightCell', subType: 'text', operator: 'containsText', value: 'a"b', style } },
      { ranges: [range], stopIfTrue: false, rule: { type: 'highlightCell', subType: 'duplicateValues', style } },
      { ranges: [range], stopIfTrue: false, rule: { type: 'highlightCell', subType: 'rank', isBottom: true, isPercent: false, value: 10, style } },
      { ranges: [range], stopIfTrue: false, rule: { type: 'highlightCell', subType: 'average', operator: 'lessThanOrEqual', style } },
      { ranges: [range], stopIfTrue: false, rule: { type: 'highlightCell', subType: 'formula', value: '=$A1>5', style } },
    ], dxfs)
    expect(xml).toContain('type="containsText" dxfId="0" priority="1" stopIfTrue="1" operator="containsText" text="a&quot;b"')
    ...
    expect(xml).toContain('type="aboveAverage" dxfId="0" priority="4" aboveAverage="0" equalAverage="1"')
    expect(xml).toContain('<cfRule type="expression" dxfId="0" priority="5"><formula>$A1&gt;5</formula></cfRule>')
  ...
  it('serializes color scales, data bars, and icon sets', () => {
    ...
    const xml = applyCfRules(SHEET, [
      ...
      {
        ranges: [range], stopIfTrue: false,
        rule: { type: 'dataBar', isShowValue: false, config: {
          min: { type: 'min' }, max: { type: 'max' }, isGradient: true,
          positiveColor: '#638EC6', nativeColor: '#FF0000',
        } },
      },
      {
        ranges: [range], stopIfTrue: false,
        rule: { type: 'iconSet', isShowValue: true, config: [
          { iconType: '3TrafficLights1', iconId: '0', operator: 'greaterThanOrEqual', value: { type: 'percent', value: 67 } },
          { iconType: '3TrafficLights1', iconId: '1', operator: 'greaterThanOrEqual', value: { type: 'percent', value: 33 } },
          { iconType: '3TrafficLights1', iconId: '2', operator: 'greaterThanOrEqual', value: { type: 'min' } },
        ] },
      },
    ], dxfs)
    expect(xml).toContain(
    ...
    expect(xml).toContain('<dataBar showValue="0"><cfvo type="min"/><cfvo type="max"/><color rgb="FF638EC6"/></dataBar>')
    expect(xml).toContain(
  ...
  it('maps the worst-first rating sets and strict thresholds', () => {
    const xml = applyCfRules(SHEET, [{
      ranges: [range], stopIfTrue: false,
      rule: { type: 'iconSet', isShowValue: true, config: [
        { iconType: '4Rating', iconId: '3', operator: 'greaterThanOrEqual', value: { type: 'percent', value: 75 } },
        { iconType: '4Rating', iconId: '2', operator: 'greaterThanOrEqual', value: { type: 'percent', value: 50 } },
        { iconType: '4Rating', iconId: '1', operator: 'greaterThan', value: { type: 'percent', value: 25 } },
        { iconType: '4Rating', iconId: '0', operator: 'greaterThanOrEqual', value: { type: 'min' } },
      ] },
    }], new FakeDxfs())
    expect(xml).toContain(
  ...
  it('flags icon configs the base schema cannot hold', () => {
    ...
    expect(iconSetSaveable([entry('3Arrows', '0'), entry('3Arrows', '1'), entry('3Arrows', '2')])).toBe(true)
    expect(iconSetSaveable([entry('4Rating', '3'), entry('4Rating', '2'), entry('4Rating', '1'), entry('4Rating', '0')])).toBe(true)
    expect(iconSetSaveable([entry('3Stars', '0'), entry('3Stars', '1'), entry('3Stars', '2')])).toBe(false)
    expect(iconSetSaveable([entry('3Arrows', '0'), entry('3Flags', '1'), entry('3Arrows', '2')])).toBe(false)
    expect(iconSetSaveable([entry('3Arrows', '0'), entry('3Arrows', '2'), entry('3Arrows', '1')])).toBe(false)
  })

  it('fails closed on Univer-only icon sets and time periods', () => {
    expect(() => applyCfRules(SHEET, [{
      ranges: [range], stopIfTrue: false,
      rule: { type: 'iconSet', config: [
        { iconType: '3Stars', iconId: '0', value: { type: 'min' } },
        { iconType: '3Stars', iconId: '1', value: { type: 'max' } },
      ] },
    }], new FakeDxfs())).toThrow(/icon set/)
    expect(() => applyCfRules(SHEET, [{
      ranges: [range], stopIfTrue: false,
      rule: { type: 'highlightCell', subType: 'timePeriod', operator: 'today', style: {} },
    }], new FakeDxfs())).toThrow(/Date-occurring/)
  })
})

const X14_EXT = '<extLst><ext uri="{78C0D931-6437-407d-A8EE-F0AAD7539E65}"'
...
const LINKED_BAR = '<conditionalFormatting sqref="C1:C5">'
...
const barRule = {
  ranges: [barRange],
  stopIfTrue: false,
  rule: { type: 'dataBar', config: { min: { type: 'min' }, max: { type: 'max' }, positiveColor: '#638EC6' } },
}
...
describe('applyCfRules with x14 extensions', () => {
  ...
  it('assigns priorities around the values x14 rules occupy', () => {
    const xml = applyCfRules(X14_SHEET, [highlight([range]), highlight([{ ...range, startColumn: 3, endColumn: 3 }])], new FakeDxfs())
  ...
  it('rejects modifying, retyping, or deleting the x14-linked data bar rule', () => {
    ...
    expect(() => applyCfRules(sheet, [recolored], new FakeDxfs())).toThrow(/data-bar extension format/)
    expect(() => applyCfRules(sheet, [highlight([barRange])], new FakeDxfs())).toThrow(/data-bar extension format/)
    expect(() => applyCfRules(sheet, [highlight([range])], new FakeDxfs())).toThrow(/data-bar extension format/)
