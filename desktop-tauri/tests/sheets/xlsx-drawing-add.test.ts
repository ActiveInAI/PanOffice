...
import {
  ...
  planCellEditsToXlsx,
} from '../../src/apps/sheets/gateway/xlsx-gateway'
import type { CellEdit, SheetVisualAddition } from '../../src/apps/sheets/gateway/xlsx-gateway'
import type { SheetEditPlan } from '../../src/apps/sheets/gateway/xlsx-sheets'
...
async function planWith(additions: SheetVisualAddition[], buffer?: Uint8Array) {
...
describe('visual additions: sheet with an existing drawing', () => {
  async function buildDrawingFixture(): Promise<Uint8Array> {
...
describe('visual additions: sheets created in the same save', () => {
  ...
  async function entryText(buffer: Uint8Array, path: string): Promise<string> {
  ...
  it('image on an added sheet writes media bytes and the Default extension', async () => {
    ...
    const media = await zip.file('xl/media/image1.png')!.async('uint8array')
    expect(Buffer.from(media.subarray(0, 4))).toEqual(Buffer.from([0x89, 0x50, 0x4e, 0x47]))
    const contentTypes = await entryText(mutation.buffer, '[Content_Types].xml')
    expect(contentTypes).toContain('Extension="png"')
    const drawingXml = await entryText(mutation.buffer, 'xl/drawings/drawing1.xml')
