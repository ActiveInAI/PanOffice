...
import { applyDvRules, DvEditError } from '../../src/apps/sheets/gateway/xlsx-dv'
...
describe('applyDvRules', () => {
  it('replaces the existing section with the snapshot, before hyperlinks', () => {
    const xml = applyDvRules(SHEET, [{
      ranges: [range],
      rule: {
        type: 'decimal', operator: 'greaterThan', formula1: '3', allowBlank: true,
      },
    }])
  ...
  it('quotes list literals and strips = from list references', () => {
    ...
    expect(xml).toContain(
      '<dataValidation type="list" showDropDown="1" sqref="B1:B5">',
    )
  ...
  it('writes prompts, errors, and the inverse error-style mapping', () => {
    const xml = applyDvRules(SHEET, [{
      ...
      rule: {
        ...
        showInputMessage: true, promptTitle: 'Limit', prompt: 'Max 10 & "short"',
        showErrorMessage: true, errorStyle: 2, errorTitle: 'Too long', error: 'Shorten it',
      },
    }])
    expect(xml).toContain(
  ...
  it('maps any back to the attribute-less default type and skips between', () => {
    ...
    expect(xml).toContain(
      '<dataValidation showInputMessage="1" prompt="Read me" sqref="A1:A5"/>',
    )
    expect(xml).toContain(
    ...
    const xml = applyDvRules(SHEET, [
      {
        ranges: [range],
        rule: {
          type: 'date', operator: 'notBetween',
          formula1: '2024-01-01', formula2: '2024-12-31 12:00:00',
        },
      },
      {
  ...
  it('fails closed on Univer-only rule types', () => {
    expect(() => applyDvRules(SHEET, [
      { ranges: [range], rule: { type: 'checkbox', formula1: '1', formula2: '0' } },
    ])).toThrow(DvEditError)
    expect(() => applyDvRules(SHEET, [
      { ranges: [range], rule: { type: 'listMultiple', formula1: 'a,b' } },
