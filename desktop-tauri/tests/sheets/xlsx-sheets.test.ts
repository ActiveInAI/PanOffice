...
import {
  ...
  validateSheetName,
} from '../../src/apps/sheets/gateway/xlsx-sheets'
...
describe('sheet duplicate save', () => {
  ...
  it('clones hyperlink relationships and drops printer settings', async () => {
    ...
    const source = await zip.generateAsync({ type: 'uint8array', compression: 'DEFLATE' })
  ...
  it('fails closed on drawings, tables, and sheet-scoped defined names', async () => {
    ...
    const withDrawing = await zip.generateAsync({ type: 'uint8array', compression: 'DEFLATE' })
