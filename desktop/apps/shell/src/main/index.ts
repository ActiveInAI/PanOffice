...
// run silently quits and forwards its argv to the running installed PanOffice.
...
if (!app.isPackaged)
  app.setPath(
    'userData',
    process.env.GENOFFICE_USER_DATA ?? join(app.getPath('appData'), 'PanOffice Dev'),
  )

// The product renames (AI Office → GenOffice → PanOffice) changed the userData
// path; migrate old user data once. First existing legacy dir wins.
if (app.isPackaged) {
  const newDir = app.getPath('userData')
  const newEmpty = !existsSync(newDir) || readdirSync(newDir).length === 0
  if (newEmpty)
    for (const legacy of ['AI Office', 'GenOffice']) {
      const oldDir = join(app.getPath('appData'), legacy)
...
// PanOffice fork — the upstream Genspark community short link was removed;
// set PANOFFICE_COMMUNITY_URL to enable the onboarding community link.
const COMMUNITY_URL = process.env.PANOFFICE_COMMUNITY_URL ?? ''
