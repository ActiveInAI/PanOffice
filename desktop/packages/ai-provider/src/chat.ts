...
import { ARCHGPT_DEFAULT_BASE_URL, GENSPARK_LLM_BASE_URLS } from './providers'
...
export async function chatForProvider(
  ...
  switch (provider) {
    ...
    case 'archgpt':
      return chatOpenAiCompatible(config.baseUrl || ARCHGPT_DEFAULT_BASE_URL, config, system, user)
    case 'custom':
