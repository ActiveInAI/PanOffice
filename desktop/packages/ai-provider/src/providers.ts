...
/**
 * Arch-GPT LLM gateway (OpenAI-compatible). Default is the local engine
 * endpoint (06-workers/engine_server.py, POST /v1/chat/completions); point
 * baseUrl at a hosted Arch-GPT gateway for deployed builds. Model list is
 * dynamic — served by the gateway's GET /v1/models.
 */
export const ARCHGPT_DEFAULT_BASE_URL = 'http://127.0.0.1:7071/v1'

export const AI_PROVIDERS: AiProviderMeta[] = [
  {
    id: 'archgpt',
    label: 'Arch-GPT',
    // models are dynamic — served by the gateway's GET /v1/models
    models: [],
    defaultModel: '',
    keyPlaceholder: 'API key / JWT (empty for local loopback)',
    needsBaseUrl: true,
  },
  {
    id: 'genspark',
...
export function defaultAiSettings(
  ...
  return { provider: 'archgpt', providers }
}
